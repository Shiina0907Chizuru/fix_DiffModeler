...
def init_save_path(params):
    ...
    now = time.strftime("%H%M%S")  # 去掉冒号，改为仅使用小时、分钟和秒
...
def configure_dataset(params):
    ...
    train_list = train_list[:train_list_len] #前面的是验证集，后面的是训练集
...
def main_worker(params):
    ...
    # 检查是否启用问题批次监控
    if 'trouble_log' in params and params['trouble_log']:
        print("启用问题批次监控功能")
        trouble_log_dir = os.path.join(log_path, 'trouble_logs')
        os.makedirs(trouble_log_dir, exist_ok=True)
        params['trouble_log_path'] = os.path.join(trouble_log_dir, 'train_trouble_log.txt')
        # 创建问题日志文件头部
        with open(params['trouble_log_path'], 'w') as f:
            f.write("=== DiffModeler Training Trouble Log ===\n")
            f.write(f"训练开始时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        print(f"问题批次监控日志将保存到: {params['trouble_log_path']}")
