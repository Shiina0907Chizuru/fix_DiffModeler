...
def gen_output_box(label_data, box_size, stride, contour, save_path):
    scan_x, scan_y, scan_z = label_data.shape
    count_voxel = 0
    count_iter = 0
    Coord_Voxel = []
    ...
    for x in range(0, scan_x, stride):
        ...
        for y in range(0, scan_y, stride):
            ...
            for z in range(0, scan_z, stride):
                ...
                # Adjust the box boundaries if they exceed the dataset dimensions
                if x_end < scan_x:
                    x_start = x
                else:
                    x_start = x_end - box_size
                ...
                # Create a box of zeros with dimensions (box_size, box_size, box_size)
                segment_label_voxel = np.zeros([box_size, box_size, box_size])
                segment_label_voxel[:x_end - x_start, :y_end - y_start, :z_end - z_start] = label_data[x_start:x_end,
                                                                                            y_start:y_end,
                                                                                            z_start:z_end]

                # Check meaningful density based on the contour level
                meaningful_density_count = len(np.argwhere(segment_label_voxel > contour))
                meaningful_density_ratio = meaningful_density_count / float(box_size ** 3)

                if meaningful_density_ratio <= 0.001:
                    continue  # Skip if there's little meaningful density

                # Save the output box
                cur_path = os.path.join(save_path, f"output_{count_voxel}.npy")
                np.save(cur_path, segment_label_voxel)
                Coord_Voxel.append([x_start, y_start, z_start])
                count_voxel += 1
