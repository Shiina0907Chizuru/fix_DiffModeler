...
class Train_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_path, train_id_list, box_size,
        ...
        for map_name in listfiles:
            ...
            cur_input_list.sort()
            cur_nuc_list.sort()
            cur_id_list = [map_name + "_" + str(os.path.split(x)[1].replace("output_", "").replace(".npy", "")) for x in
                           cur_nuc_list]
    ...
    def __getitem__(self, idx):
        ...
        if len(input.shape) == 3:  # (D, H, W)
            input = input[np.newaxis, :]  # (1, D, H, W)
        input = torch.from_numpy(
            np.array(input, np.float32, copy=True))  # 将 NumPy 数组 input 转换为 PyTorch 的张量，并确保数据类型为 float32
        atom_final_input = []
